...
var require_tr46 = __commonJS({
  'node_modules/tr46/index.js'(exports2, module2) {
    ...
    function processing(domain_name, useSTD3, processing_option) {
      ...
      var labels2 = result.string.split('.');
      for (var i = 0; i < labels2.length; ++i) {
        try {
          var validation = validateLabel(labels2[i]);
          labels2[i] = validation.label;
    ...
    module2.exports.toASCII = function (domain_name, useSTD3, processing_option, verifyDnsLength) {
      ...
      var labels2 = result.string.split('.');
      labels2 = labels2.map(function (l) {
        try {
      ...
      if (verifyDnsLength) {
        var total = labels2.slice(0, labels2.length - 1).join('.').length;
        if (total.length > 253 || total.length === 0) {
          result.error = true;
        }
        for (var i = 0; i < labels2.length; ++i) {
          if (labels2.length > 63 || labels2.length === 0) {
            result.error = true;
            break;
          }
        }
...
var require_labels = __commonJS({
  'src/labels.js'(exports2, module2) {
    ...
    async function listLabelsForRepo(octokit2) {
      ...
      await octokit2
        .paginate(octokit2.rest.issues.listLabelsForRepo, {
          owner,
          repo
        })
        .then(labels2 => {
      ...
      repository(owner: "${owner}", name: "${repo}") {
        ...
        if (deployableType2 && deployableType2.length > 0) {
          const issues = response.repository.issues.edges
            .filter(ri => ri.node.title.toLowerCase().includes(deployableType2.toLowerCase()))
...
|${project2.columnName}|${workflowUrl}|${labels2.deployStatus}|${nowString}|${actor2}|${labels2.deployLabel != null ? labels2.deployLabel : 'NA'}|`;
      ...
      await octokit2.rest.issues
        .createComment({
          owner,
          repo,
          issue_number: issueToUpdate2.number,
