  ...
  "src/projects.js"(exports2, module2) {
    ...
    query {
      repository(owner: "${owner}", name: "${repo}") {
        ...
        if (!response.repository.project || !response.repository.project.columns || !response.repository.project.columns.edges || !response.repository.project.columns.edges.length === 0) {
          core2.setFailed(`Project board #${projectToUpdate.number} does not appear to be set up correctly. Make sure you are using the correct board number and that the ${projectToUpdate.columnName} column exists.`);
        ...
        const matchingColumn = response.repository.project.columns.edges.find((c) => c.node.name.toLowerCase() === projectToUpdate.columnName.toLowerCase());
        if (!matchingColumn) {
  ...
  "src/labels.js"(exports2, module2) {
    ...
    async function removeStatusLabelsFromIssue2(octokit2, existingLabels, issueNumber, deployStatus2) {
      try {
        ...
        if (hasSuccessLabel && !currentStatusIsSuccess)
          await removeLabelFromIssue2(octokit2, "success", issueNumber);
        if (hasFailureLabel && !currentStatusIsFailure)
          await removeLabelFromIssue2(octokit2, "failure", issueNumber);
        if (hasCancelledLabel && !currentStatusIsCancelled)
          await removeLabelFromIssue2(octokit2, "cancelled", issueNumber);
        if (hasSkippedLabel && !currentStatusIsSkipped)
          await removeLabelFromIssue2(octokit2, "skipped", issueNumber);
    ...
    async function findIssuesWithLabel2(graphqlWithAuth2, labelName, projectNumber) {
      try {
    ...
    query {
      repository(owner: "${owner}", name: "${repo}") {
        ...
        const issuesPerBoard = response.repository.issues.edges.filter((issueEdge) => issueEdge.node.projectCards.length > 0 && issue.edges.node.projectCards.edges.filter((projectEdge) => projectEdge.node.project.number == projectNumber).length > 0).map((ri) => ri.node.number);
        const issues = response.repository.issues.edges.filter((ri) => projectNumber === 0 || issuesPerBoard.includes(ri.node.number)).map((ri) => ri.node.number);
  ...
  "src/issues.js"(exports2, module2) {
    ...
    query {
      repository(owner: "${owner}", name: "${repo}") {
        ...
        const existingIssues = response.repository.issues.edges.filter((issue2) => issue2.node.title.toLowerCase() === issueToUpdate2.title.toLowerCase());
        if (!existingIssues || existingIssues.length === 0) {
          core2.info(`An issue with the title '${issueToUpdate2.title}' was not found.`);
          core2.endGroup();
          return;
        }
        let issuesOnAnyProjectBoard = existingIssues.filter((e2) => e2.node && e2.node.projectCards && e2.node.projectCards.edges && e2.node.projectCards.edges.length > 0);
        ...
        let existingIssue = issuesOnAnyProjectBoard.find((i) => i.node.projectCards.edges.find((pc) => pc.node.project && pc.node.project.databaseId == projectBoardId2));
        if (!existingIssue) {
